...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE year = 2014 AND round ILIKE 'Eighth-Final' UNION SELECT name FROM games INNER JOIN teams ON games.opponent_id = teams.team_id WHERE year = 2014 AND round ILIKE 'Eighth-Final' ORDER BY name")"
